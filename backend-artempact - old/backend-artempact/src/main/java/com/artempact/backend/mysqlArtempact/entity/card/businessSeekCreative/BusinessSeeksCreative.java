package com.artempact.backend.mysqlArtempact.entity.card.businessSeekCreative;
...
public class BusinessSeeksCreative extends Card {
    ...
    @NotBlank(message = "The identifyCreativeType cannot be empty")
    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "identifyCreativeType_id", referencedColumnName = "id")
    private TypeOfCreative identifyCreativeType;

    @Embedded
    private BusinessSeeksCreative.LocationSelection locationSelection;

    public BusinessSeeksCreative(String title, String description, Integer setMinProjectBudget, Integer setMaxProjectBudget, String cardColor, EducationType educationalBackground, ExperienceLevel experienceLevel, ProfessionalRelationship professionalRelationship, String companyVisionMission, TypeOfCreative identifyCreativeType, LocationSelection locationSelection) {
